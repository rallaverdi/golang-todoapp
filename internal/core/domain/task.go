...
func (t *Task) Validate() error {
	...
	if t.Completed {
		if t.CompletedAt == nil {
			return fmt.Errorf("`CompletedAt` can not be `nil` if `Completed` is `True`: %w", core_errors.ErrInvalidArgument)
		}
...
func (p *TaskPatch) Validate() error {
	if p.Title.Set && p.Title.Value == nil {
		return fmt.Errorf("`Title` can not be `NULL`: %w", core_errors.ErrInvalidArgument)
	}

	if p.Completed.Set && p.Completed.Value == nil {
		return fmt.Errorf("`Completed` can not be `NULL`: %w", core_errors.ErrInvalidArgument)
	}
