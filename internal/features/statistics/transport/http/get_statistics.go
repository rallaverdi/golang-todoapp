...
func (h *StatisticsHTTPHandler) GetStatistics(rw http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		responseHandler.ErrorResponse(
			err,
			"failed to get user id from getQueryParams",
		)
		return
	}

	statisticDomains, err := h.statisticsService.GetStatistics(ctx, userID, from, to)
	if err != nil {
		responseHandler.ErrorResponse(
			err,
			"failed to get statistics for user",
		)
		return
	}

	response := toDTOFromDomain(statisticDomains)
...
//	to     *time.Time
//}
// getUserIDFromQueryParams () (queryParams, error)
